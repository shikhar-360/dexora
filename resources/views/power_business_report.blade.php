    ...
    <div class="container-fluid">
        <div class="row bg-title">
            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                <h4 class="page-title">Power Business Report</h4>
            </div>
        </div>
                    ...
                    <div>
                        ...
                        <ul class="nav nav-tabs member-tab" role="tablist">
                            <li role="presentation" class="active"><a href="#all_members" aria-controls="all_members"
                                    role="tab" data-toggle="tab">Power Business Report</a></li>
                        </ul>
                        ...
                        <div class="tab-content">
                            <div role="tabpanel" class="tab-pane active" id="all_members">
                                <div class="search-form export-form">
                                    <form action="{{route('power_business_report')}}" method="post" class="mb-0">
                                        @csrf
                                        <div class="form-row">
                                                        ...
                                                        <thead class="table-light">
                                                            <tr>
                                                               <!--  <th scope="col" class="border-0 fs-4 font-weight-medium ps-4">
                                                                ...
                                                                <th scope="col" class="border-0 fs-4 font-weight-medium">Strong Business</th>
                                                                ...
                                                                <th scope="col" class="border-0 fs-4 font-weight-medium">Power Business Date</th>
                                                                <th scope="col" class="border-0 fs-4 font-weight-medium">Action</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                                <!-- <td class="ps-4">
                                                            ...
                                                            @foreach($data['data']['data'] as $key => $value)
                                                                <tr>
                                                                    ...
                                                                    <td>{{ $value['refferal_code'] }}</span></td>
                                                                    <td>{{ substr($value['wallet_address'], 0, 6) . '...' . substr($value['wallet_address'], -6) }}</span></td>
                                                                    <td><span>{{$value['sponser_code']}}</span></td>
                                                                    <td><span>{{$value['strong_business']}}</span></td>
                                                                    <td><span>{{$value['rank']}}</span></td>
                                                                    <td><span>{{date('d-m-Y H:i', strtotime($value['created_on']))}}</span></td>
                                                                    @if($value['power_date']!=NULL)
                                                                    <td><span>{{date('d-m-Y', strtotime($value['power_date']))}}</span></td>
                                                                    @else
                                                                    <td><span>-</span></td>
                                                                    @endif
                                                                    <td><form action="{{route('remove_power_business')}}" method="POST">
                                                                        @method('post')
                                                                        @csrf
                                                                        <input type="hidden" name="user_id" value="{{$value['id']}}">

                                                                        <input type="submit" name="submit" value="Remove Power Business" class="btn btn-danger">
                                                                    </form>
                                                                    </td>
                                                                </tr>
                                                            @endforeach
