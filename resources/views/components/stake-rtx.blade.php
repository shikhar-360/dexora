<div class="my-4 space-y-6">
    ...
    <div class="bg-[#1c1a39] rounded-lg p-4 md:p-5 border border-[#322751]">
        <div class="mb-5">
            <h3 class="text-lg font-semibold text-white mb-2">Add to Your Stake</h3>
            <p class="text-gray-400">Increase your staked amount to earn more rewards.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div class="bg-[#20202a] rounded-lg p-4 border-l-4 border-green-500">
                ...
                <p class="text-xs text-green-400">Active user • Auto-claimed on new stakes</p>
            </div>

            <div class="bg-[#20202a] rounded-lg p-4">
                <p class="text-sm text-gray-400 mb-1">Current Stake Duration</p>
                <p class="text-lg font-semibold text-white flex items-center gap-2">
                    {{$data['last_stake_days']}} <span class="text-sm text-gray-400">days</span>
                </p>
                <p class="text-xs text-gray-500">Last staked: {{date('d F, Y H:i', strtotime($data['last_stake']['created_on']))}}</p>
            </div>
        </div>
        @endif

        <button
            onclick="document.getElementById('stakemore-modal').classList.remove('hidden')"
    ...
    <div class="bg-blue-900/10 border border-blue-700/30 rounded-lg p-4">
        <h4 class="text-sm font-semibold text-blue-400 mb-2">Important Information</h4>
        <ul class="text-xs text-blue-300 space-y-1 list-disc list-inside">
            <li>Minimum stake amount: 1 USDT ( ~ {{$data['rtxPrice']}} RTX )</li>
            <li>Rewards start accumulating immediately but can only be claimed after 1 day</li>
            <li>You can add to your stake at any time</li>
        </ul>
    </div>
</div>
<div id="stakemore-modal" class="fixed inset-0 z-[9999] bg-black/60 hidden flex items-center justify-center">
    <div class="bg-[#1a1a23] rounded-xl p-6 w-full max-w-md border border-[#322751] shadow-2xl overflow-auto" style="max-height: 97vh;">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold text-white">Add to Stake</h2>
            <button onclick="document.getElementById('stakemore-modal').classList.add('hidden')" class="text-gray-400 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed">✕</button>
        </div>
        <div class="space-y-4">
            <form class="space-y-4" id="processPackage" method="post" action="{{route('fhandlePackageTransaction')}}">
                ...
                @csrf
                <div class="bg-[#20202a] rounded-lg p-4">
                    <div class="flex justify-between text-sm"><span class="text-gray-400">Available Balance:</span><span class="text-white font-medium availableRTX" id="availableRTX">0 RTX</span></div>
                </div>
                ...
                <div class="space-y-3 pt-4">
                    ...
                    <input type="hidden" name="lock_period" id="lock_period" value="0">
                    <button class="w-full bg-gradient-to-t from-[#6b3fb9] to-indigo-500 border border-none hover:border-[#bd97ff] disabled:opacity-50 disabled:cursor-not-allowed text-white py-3 rounded-lg font-medium transition-colors"
                        tabindex="0" type="button" onclick="activatePackage(this);" id="proccessButton">Process</button>
                    <button onclick="document.getElementById('stakemore-modal').classList.add('hidden')" type="button" class="w-full bg-transparent border border-[#322751] hover:bg-[#322751] disabled:opacity-50 disabled:cursor-not-allowed text-gray-300 py-3 rounded-lg font-medium transition-colors">Cancel</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    async function activatePackage(btn) {
        ...
        if (amount < 1) {
            showToast("error", "Minimum Investment Amount is $1.");
            btn.disabled = false;
            return;
        }
            ...
            try {
                ...
                if (parseFloat(allowance) > parseFloat((amount.toString()))) {
                    ...
                    try {
                        ...
                        txn = await buyContractInst.stake(referrer, usdtAmount, 1, 0);
                    } catch (err) {
                        btn.innerHTML = 'Stake';
                        btn.disabled = false;
                        console.log(err['data'] ? err['data']['message'] : err['reason'] ? err['reason'] : err['message']);
                        swal("Package Activation", "Error while processing the transaction! Check balance, gas fees and try again.\n\n"+(
                            err['data'] ? err['data']['message'] : err['reason'] ? err['reason'] : err['message']
                        ), "error")
                        return {
                            txn,
                            isErrorWithin: true
                    ...
                    // let txnBuy;
                    // try {
                    //     console.debug(usdtAmount, referrer, 1, 0);
                    //     let gasPrice = await signer.provider.getGasPrice()
                    //     txnBuy = await buyContractInst.stake(referrer, usdtAmount, 1, 0);
                    // } catch (err) {
                    //     btn.disabled = false;
    ...
    async function processUnstake(btn) {
        ...
        try {
            ...
            txn = await buyContractInst.withdraw(usdtAmount, 1);
            showToast("info", "Confirming transaction.. Please Don't refresh the page!!!");
