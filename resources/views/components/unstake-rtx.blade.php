            ...
            <table id="withdrawalsTable" class="w-full text-left border-collapse" style="padding-top: 15px;">
                <thead>
                    <tr class="bg-white bg-opacity-10 text-white">
                        ...
                        <th class="px-4 py-2">ROI Generated</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap px-4 py-2">{{ isset($data['earnings'][$key]) ? number_format($data['earnings'][$key], 3) : 0 }}</td>
    ...
    <div class="bg-[#1a1a23] rounded-xl p-6 w-full max-w-md border border-[#322751] shadow-2xl overflow-auto" style="max-height: 97vh;">
        ...
        <form class="space-y-5" action="{{route('fwithdrawProcess')}}" id="unstake-process-form" method="post">
            ...
            <div class="flex gap-3">
                ...
                <button type="button" class="flex-1 bg-gradient-to-r from-[#845dcb] to-[#6b3fb9] hover:from-[#6b3fb9] hover:to-[#5a2fa8] disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed text-white font-medium py-3 px-4 rounded-lg transition-all flex items-center justify-center" disabled onclicked="processWithdraw(this);">Process</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="bg-[#1a1a23] rounded-xl p-6 w-full max-w-md border border-[#322751] shadow-2xl overflow-auto" style="max-height: 97vh;">
        ...
        <form class="space-y-5" action="{{route('fwithdrawProcess')}}" id="unstake-process-form" method="post">
            ...
            <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-300">Amount</label>
                <div class="relative">
                    <input placeholder="Enter amount" id="yourunstakeamount" name="amount" class="w-full bg-[#1a1a23] border border-[#322751] rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#845dcb] focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
                        type="text">
                </div>
            </div>
            <div class="flex gap-3">
                ...
                <input type="hidden" name="withdraw_type" value="UNSTAKE" id="withdraw_type">
                <input type="hidden" name="package_id" value="1" id="package_id">
                ...
                .then(async (confirmed) => {
                    ...
                    if (confirmed) {
                        let gasPrice = await signer.provider.getGasPrice()
                            txn = await buyContractInst.withdrawFromRewards(usdtAmount);
                            return txn
                    } else {
                        return null
                    }
